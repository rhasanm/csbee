...
use crate::{app::{App, InputMode}, handler::{handle_filter_key_event, handle_order_key_event, handle_query_key_event, handle_schema_key_event, handle_table_key_event}};
...
impl EventHandler {
    ...
    fn handle_key_event(&mut self, key_event: KeyEvent, app: &mut App) {
        match app.input_mode {
            ...
            InputMode::Query => {}
            InputMode::Order if key_event.kind == KeyEventKind::Press => match key_event.code {
                _ => handle_order_key_event(key_event, app)
            }
            InputMode::Order => {}
